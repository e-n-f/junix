...
int getopt_json(int argc, char **argv, std::string &getopt_str, struct option *long_options, std::vector<flag> &flags, std::vector<const char *> &to_free) {
	int i;
...
std::string make_options(struct option *long_options) {
...
int main(int argc, char **argv) {
	...
	std::string getopt_str = make_options(long_options);

	bool want_long = false;
	bool recursive = false;

	int i;
	while ((i = getopt_json(argc, argv, getopt_str, long_options, flags, to_free)) != -1) {
		switch (i) {
		case 'l':
			want_long = true;
			break;
